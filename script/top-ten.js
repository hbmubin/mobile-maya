...
const displayProducts = (products = productCards) => {
  ...
  for (let i = start_index; i <= end_index; i++) {
    if (products[i]) {
      statement += `<div class="card w-full product-card grid sm:grid-cols-3 grid-cols-5 sm:grid-rows-6 grid-rows-7 sm:pr-3 pr-2">${products[i].innerHTML}</div>`;
    }
  }
