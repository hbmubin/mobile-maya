...
const displayProducts = (products = productCards) => {
  ...
  for (let i = start_index; i <= end_index; i++) {
    if (products[i]) {
      statement += `<a href="./news.html" class="product-card border rounded-r-md bg-white flex md:flex-row flex-col  border-t-2">${products[i].innerHTML}</a>`;
    }
  }
